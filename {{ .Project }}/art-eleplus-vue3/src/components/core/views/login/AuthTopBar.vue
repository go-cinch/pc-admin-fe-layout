...
<template>
  ...
  >
    <div class="flex-cc !hidden max-[1180px]:!flex ml-2 max-sm:ml-6">
      ...
      <h1 class="auth-mobile-title font-medium ml-2">
        <span class="title-xs">Go Cinch</span>
        <span class="title-sm">Go Cinch Admin</span>
      </h1>
    </div>
    ...
    <div class="flex-cc gap-1.5 mr-2 max-sm:mr-5">
      ...
      <button
        type="button"
        class="btn h-8 w-8 c-p flex-cc tad-300"
        :aria-label="$t('auth.panelPosition')"
        :title="$t('auth.panelPosition')"
        @click="togglePanelPosition"
      >
        <ArtSvgIcon
          :icon="panelPosition === 'right' ? 'ri:layout-right-2-line' : 'ri:layout-left-2-line'"
          class="text-xl text-g-800 transition-colors duration-300"
        />
      </button>
      <ElDropdown
...
<script setup lang="ts">
  ...
  const panelPosition = ref<'left' | 'right'>(
    localStorage.getItem('art-auth-panel-position') === 'left' ? 'left' : 'right'
  )

  const applyPanelPosition = () => {
    document.documentElement.dataset.authPanel = panelPosition.value
  }

  onMounted(applyPanelPosition)

  const togglePanelPosition = () => {
    panelPosition.value = panelPosition.value === 'right' ? 'left' : 'right'
    localStorage.setItem('art-auth-panel-position', panelPosition.value)
    applyPanelPosition()
  }
