  ...
  >
    ...
    >
      ...
      <div class="flex-c gap-2.5">
        ...
        <!-- 高级配置 -->
        <div v-if="shouldShowSettings" class="action-settings">
        ...
        <!-- 时区 -->
        <ElDropdown class="action-timezone" @command="changeTimezone">
          <ArtIconButton icon="ri:time-zone-line" :title="$t('topBar.timezone')" />
          <template #dropdown>
            <ElDropdownMenu>
              <ElDropdownItem
        ...
        <ArtIconButton
          ...
          class="action-fullscreen max-md:!hidden"
        ...
        <ArtIconButton
          class="action-lock"
          icon="ri:lock-line"
          :title="$t('topBar.user.lockScreen')"
          @click="openLockScreen"
        />
...
<script setup lang="ts">
  ...
  const timezone = ref(localStorage.getItem('art-timezone') || browserTimezone)
  const timezoneOptions = computed(() =>
    [...new Set([browserTimezone, 'Asia/Shanghai', 'UTC'])].map((value) => ({
      label: value === browserTimezone ? `${value} (${t('topBar.localTimezone')})` : value,
      value
    }))
  )
  ...
  const changeTimezone = (value: string): void => {
    ...
    localStorage.setItem('art-timezone', value)
    window.dispatchEvent(new CustomEvent('art-timezone-change', { detail: value }))
  }

  const openLockScreen = (): void => {
