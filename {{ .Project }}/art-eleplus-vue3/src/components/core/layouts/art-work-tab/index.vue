    ...
    <div class="w-full overflow-hidden" ref="scrollRef">
      ...
      >
        ...
        >
          ...
          <button
            type="button"
            class="tab-pin ml-1 opacity-0 group-hover:opacity-100 focus:opacity-100"
            :aria-label="$t(item.fixedTab ? 'worktab.btn.unfixed' : 'worktab.btn.fixed')"
            :title="$t(item.fixedTab ? 'worktab.btn.unfixed' : 'worktab.btn.fixed')"
            @click.stop="useWorktabStore().toggleFixedTab(item.path)"
          >
            <ArtSvgIcon
              :icon="item.fixedTab ? 'ri:pushpin-fill' : 'ri:pushpin-2-line'"
              class="text-xs"
            />
          </button>
          <span
