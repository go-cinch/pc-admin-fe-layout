...
import { logout as logoutApi, type SessionResult } from '@/api/auth-service'
...
export const useUserStore = defineStore(
  ...
  () => {
    ...
    const setLanguage = (lang: LanguageEnum) => {
      language.value = lang
      nextTick(() => setPageTitle(router.currentRoute.value))
    }
    ...
    const acceptSession = (session: SessionResult) => {
      ...
      passwordResetRequired.value = session.password_reset_required
      isLogin.value = true
    }
    ...
    const logOut = async () => {
      ...
      if (tokenToRevoke) {
        try {
          ...
          // Local session cleanup must still happen when the server is unavailable.
        }
      }
