...
export const useSettingStore = defineStore(
  ...
  () => {
    ...
    /** 版权设置（认证页与登录页共享） */
    const copyrightEnabled = ref(true)
    const copyrightDate = ref(String(new Date().getFullYear()))
    const copyrightCompany = ref('go-cinch')
    const copyrightCompanyLink = ref('https://github.com/go-cinch/demos')
