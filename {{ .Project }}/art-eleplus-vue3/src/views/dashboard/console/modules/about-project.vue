<template>
  <div class="art-card p-5 flex-b mb-5 max-sm:mb-4">
    <div>
      <h2 class="text-2xl font-medium">{{ $t('dashboardContent.console.aboutProject') }}</h2>
      <p class="text-g-700 mt-1">{{
        $t('dashboardContent.console.aboutDescription').replace('{name}', systemName)
      }}</p>
      <p class="text-g-700 mt-1">{{ $t('dashboardContent.console.technologyDescription') }}</p>
...
<script setup lang="ts">
  ...
  const linkList = computed(() => [
    { label: $t('dashboardContent.console.projectWebsite'), url: WEB_LINKS.DOCS },
    { label: $t('dashboardContent.console.documentation'), url: WEB_LINKS.INTRODUCE },
    { label: 'Github', url: WEB_LINKS.GITHUB_HOME },
    { label: $t('dashboardContent.console.bilibili'), url: WEB_LINKS.BILIBILI }
  ])
