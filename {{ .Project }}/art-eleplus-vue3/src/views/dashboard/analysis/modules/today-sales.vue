<template>
  <div class="art-card h-82 p-5 mb-5 overflow-hidden max-lg:h-auto max-sm:mb-4">
    <div class="art-card-header pr-0">
      <div class="title">
        <h4>{{ $t('dashboardContent.analysis.todaySales') }}</h4>
        <p>{{ $t('dashboardContent.analysis.salesSummary') }}</p>
      </div>
      <div class="flex-cc h-7.5 min-w-17 border border-g-300 rounded-lg text-g-500 c-p">
        ...
        <span class="text-xs">{{ $t('dashboardContent.common.export') }}</span>
      </div>
    </div>
          ...
          >
            ...
            <div class="max-sm:ml-4 mt-3.5 max-sm:mt-0 max-sm:text-end">
              ...
              <small class="text-g-500 mt-1 max-sm:mt-0.5">
                {{ $t('dashboardContent.common.vsYesterday') }}
                <span
                  class="font-medium"
...
<script setup lang="ts">
  ...
  const salesData = computed<SalesDataItem[]>(() => [
    {
      label: $t('dashboardContent.analysis.totalSales'),
      value: 999,
      change: '+10%',
    ...
    {
      label: $t('dashboardContent.analysis.totalOrders'),
      value: 300,
      change: '+15%',
    ...
    {
      label: $t('dashboardContent.analysis.productsSold'),
      value: 56,
      change: '-5%',
    ...
    {
      label: $t('dashboardContent.analysis.newCustomers'),
      value: 68,
      change: '+8%',
