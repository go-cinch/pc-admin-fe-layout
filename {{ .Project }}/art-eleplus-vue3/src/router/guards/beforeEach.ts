...
async function handleRouteGuard(
  ...
  if (userStore.isLogin && userStore.passwordResetRequired) {
    if (to.path !== '/auth/reset-password') {
      next({ path: '/auth/reset-password', replace: true })
      return
    }
    // Restricted sessions may render only the reset screen. Initializing
    // dynamic routes here would call /auth/info, which is intentionally 403
    // until the required password reset succeeds.
    setPageTitle(to)
    next()
    return
  }

  if (userStore.isLogin && !userStore.passwordResetRequired && to.path === '/auth/reset-password') {
