...
> React 19 admin dashboard template · Vite · Tailwind CSS v4 · react-i18next · ApexCharts
...
- **react-i18next v17** + **i18next v26** for English and Chinese localization.
...
- **Supported locales**: exactly `en` and `zh`. Both share the single `"common"` namespace.
- **Translation dictionaries**: `src/locales/<locale>/common.json`. When adding any user-facing string, add the key to **both** locale files.
- **Using translations**:
...
- API requests must send the selected language as `Accept-Language` using the backend's `en-US` and `zh-CN` convention.
...
- Don't hardcode user-facing text — add keys to both `src/locales/<locale>/common.json` files and use `t()`.
