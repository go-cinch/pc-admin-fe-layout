...
const emit = defineEmits<{
  change: [Array<CaptchaPoint>];
  click: [CaptchaPoint];
  confirm: [Array<CaptchaPoint>, clear: () => void];
  refresh: [];
}>();
const { addPoint, clearPoints, points, removePoint } = useCaptchaPoints();
...
function removeSelectedPoint(index: number) {
  removePoint(index);
  emitPoints();
}
  ...
  try {
    ...
    const selectedIndex = points.findIndex((point) => {
      ...
      return (
        ...
        POINT_TOGGLE_RADIUS * POINT_TOGGLE_RADIUS
      );
    });
    if (selectedIndex >= 0) {
      removeSelectedPoint(selectedIndex);
      e.stopPropagation();
      e.preventDefault();
      return;
    }
...
<template>
  ...
  >
    ...
    <div
      ...
      class="absolute z-20 flex-center size-5 cursor-pointer rounded-full border-2 border-primary-50 bg-primary text-primary-50"
      role="button"
      tabindex="0"
      @click.stop.prevent="removeSelectedPoint(index)"
      @keydown.enter.prevent="removeSelectedPoint(index)"
      @keydown.space.prevent="removeSelectedPoint(index)"
    >
      {{ index + 1 }}
    </div>
    <template #footer>
