...
const vbenDescriptionItems: DescriptionItem[] = [
  ...
  {
    content: renderLink('https://github.com/go-cinch', 'go cinch'),
    title: '作者',
  },
];

const dependenciesItems = Object.keys(dependencies)
  .filter((key) => !key.startsWith('@vben'))
  .map((key) => ({
    content: dependencies[key],
    title: key,
  }));

const devDependenciesItems = Object.keys(devDependencies)
  .filter((key) => !key.startsWith('@vben'))
  .map((key) => ({
