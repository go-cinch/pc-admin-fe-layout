...
export function BarGraph() {
  ...
  const chartConfig = {
    desktop: { label: pick('Desktop', '桌面端'), color: 'var(--chart-1)' },
    mobile: { label: pick('Mobile', '移动端'), color: 'var(--chart-2)' }
  } satisfies ChartConfig;
  return (
    <Card>
      <CardHeader>
        <CardTitle>
          {pick('Monthly visits', '月访问量')}
          <Badge variant='outline'>
            <Icons.trendingDown />
            -5.2%
          </Badge>
        </CardTitle>
        <CardDescription>{pick('January – June 2025', '2025 年 1 月至 6 月')}</CardDescription>
      </CardHeader>
      <CardContent>
